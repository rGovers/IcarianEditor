...
source "build.sh" "$@" --platform=windows --remote --disable-mod --enable-profiler -R --rebuild
if [ $? -ne 0 ]; then
...
source "build.sh" "$@" --platform=linux --remote --disable-mod --enable-profiler -R --rebuild
if [ $? -ne 0 ]; then
